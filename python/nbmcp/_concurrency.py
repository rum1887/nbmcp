...
def _shutdown_process_pool():
    # Note: this runs on normal interpreter exit, but NOT on SIGKILL and not
    # reliably on SIGTERM (Python's default SIGTERM disposition terminates
    # the process without unwinding to run atexit hooks). A process killed
    # by an external supervisor can leave workers as orphans holding
    # inherited file descriptors (notably stdout/stderr) open. If you run
    # nbmcp under a process manager, prefer sending SIGINT (which Python
    # turns into a normal KeyboardInterrupt / clean unwind) over SIGTERM/-9
    # where you can.
